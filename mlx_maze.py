...
class MlxMaze:
	def __init__(self, maze_gen: MazeGenerator) -> None:
		...
		self.mlx_maze = Mlx()
		self.mlx_ptr = self.mlx_maze.mlx_init()
		self.cell_size = 20
		self.win_ptr = self.mlx_maze.mlx_new_window(self.mlx_ptr, self.width * self.cell_size + self.cell_size // 10, self.height * self.cell_size + self.cell_size // 10, 'Amazing')
	...
	def draw_rectangle(self, x: int, y: int, w: int, h: int, color: int) -> None:
		for j in range(y, y + h):
			for i in range(x, x + w):
				self.mlx_maze.mlx_pixel_put(self.mlx_ptr, self.win_ptr, i, j, color)
	...
	def draw_cell(self, x: int, y: int, wall_value: int, color: int) -> None:
		...
		if wall_value == 15:
			self.draw_rectangle(px, py, self.cell_size, self.cell_size, 0XFFFF0099)
			return

		if wall_value & 1 == 1:
			self.draw_rectangle(px, py, self.cell_size, wall_size, color)
		if wall_value & 2 == 2 and wall_value & 4 == 4:
			self.draw_rectangle(px + self.cell_size, py, wall_size, self.cell_size + wall_size, color)
		elif wall_value & 2 == 2:
			self.draw_rectangle(px + self.cell_size, py, wall_size, self.cell_size, color)
		if wall_value & 4 == 4:
			self.draw_rectangle(px,  py + self.cell_size, self.cell_size, wall_size, color)
		if wall_value & 8 == 8:
	...
	def draw_entry(self) -> None:
		x, y = self.entry
		self.draw_rectangle(x * self.cell_size, y * self.cell_size, self.cell_size, self.cell_size, 0xFF60A5FA)
	
	def draw_exit(self) -> None:
		x, y = self.exit_
		self.draw_rectangle(x * self.cell_size, y * self.cell_size, self.cell_size, self.cell_size, 0xFFFB7185)
	...
	def draw_path(self) -> None:
		for x, y in self.result:
			...
			if (x, y) in [self.entry, self.exit_]:
				continue
			self.draw_rectangle(px, py, self.cell_size, self.cell_size, 0XFF22C55E)
	...
	def animate_path(self, param) -> None:
		...
		if (x, y) not in [self.entry, self.exit_]:
			self.draw_rectangle(x * self.cell_size, y * self.cell_size, self.cell_size, self.cell_size, 0XFF22C55E)
			self.draw_cell(x, y, self.grid[y][x], self.wall_colors[0])
		self.path_animation_index += 1
	
	def animate_maze(self, param) -> None:
	...
	def key_hook(self, keycode: int, param) -> None:
		if keycode == 65307:
		...
		if keycode in [ord('p'), ord('P')]:
			if not self.is_touch_p:
				self.mlx_maze.mlx_loop_hook(self.mlx_ptr, self.animate_path, None)
		...
		if keycode == 65361:
			...
			self.draw_rectangle(px, py, self.cell_size, self.cell_size, 0X8822C55E)
